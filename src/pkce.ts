function getCrypto() {
  // @ts-ignore: browser-specific crypto lookup
  return window.crypto || window.msCrypto;
}

function getSubtle() {
  const crypto = getCrypto();
  // @ts-ignore: browser-specific subtle lookup
...
function supportsSha256() {
  ...
  return (
    ...
    // @ts-ignore: Skip old version of subtle.digest on IE that returns a CryptoOperation
